...
ig.System.inject({
    startRunLoop() {
        this.frame = 0
        this.stopRunLoop()
        if (!multi.headless && window.requestAnimationFrame) {
            window.requestAnimationFrame(drawLoop)
        }
        this.intervalId = setInterval(this.run.bind(this), 1e3 / multi.server.s.globalTps) as unknown as number
...
// let dc = 0
function drawLoop() {
    if (!ig.system.hasFocusLost() && !ig.game.fullyStopped && !multi.headless && ig.perf.draw) {
        ig.game.draw()
        ig.game.finalDraw()
    }
