...
declare global {
    namespace sc {
        interface PauseScreenGui {
            multibakeryVersionGui: sc.TextGui
        }
    }
}
prestart(() => {
    sc.PauseScreenGui.inject({
        init(...args) {
            ...
            this.multibakeryVersionGui = gui
        ...
        doStateTransition(name, skipTransition, removeAfter, callback, initDelay) {
            ...
            if (name == 'DEFAULT') {
                if (multi.server) {
                    this.multibakeryVersionGui.doStateTransition('DEFAULT')
                } else {
                    this.multibakeryVersionGui.doStateTransition('HIDDEN', true)
                }
            }
        },
    })
